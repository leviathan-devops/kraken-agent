...
export default async function SharkAgent(input) {
    ...
    const { executionBrain, reasoningBrain, systemBrain, concurrencyManager, } = initializeTripleBrain(workspacePath);
    // Load Shark identity
    const sharkIdentityPrompt = getSharkIdentityPrompt();
    // Linear hooks setup with identity injection
    const hooks = createSharkHooks(guardian, gm, ec, stateStore, messenger, sharkIdentityPrompt, SHARK_PLUGIN_IDENTITY);
