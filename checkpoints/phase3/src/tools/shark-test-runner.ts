 ...
 * Container tests must pass at 96%+ success rate before VERIFY gate.
...
function runOpenCode(command: string, timeout: number = 30000): { output: string; error: string | null } {
  ...
  try {
    const output = require('child_process').execSync(
      `${opencodeBin} run "${command}" --agent shark -m opencode/big-pickle 2>&1`,
      {
        timeout,
        encoding: 'utf-8',
    ...
    return { output: output as string, error: null };
  } catch (error) {
