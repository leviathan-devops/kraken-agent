...
export function safeHook<I = unknown, O = unknown>(
  ...
  return async (input: I, output: O): Promise<void> => {
    ...
    // Resolve agent name: some opencode versions don't set ctx.agentName
    // Fallback to input.agent which is reliably available
    // v1.14.48 has nested input: { input: { agent, ... }, output: { ... }, ctx: { ... } }
    const resolvedAgentName = ctx.agentName ?? (input as any)?.input?.agent ?? (input as any)?.agent ?? '';
